...
??? abstract "Introduktion til kapitlet"
    ...
    **--- OBS: DETTE KAPITEL ER IKKE FÆRDIGSKREVET ENDNU ---**
...
Granular er et komplekst emne med mange muligheder og parametre. Her introduceres grundlæggende teknikker og kompositionsmuligheder. For videregående teknikker henvises nysgerrige læsere til Eli Fieldsteels video nr. [25](https://www.youtube.com/watch?v=WBqAM_94TW4) og [26](https://www.youtube.com/watch?v=MnD8stNB5tE).

## Grundlæggende GrainBuf

Man kan i princippet danne grains af hvilken som helst lydkilde, men samples eller live-lydsignaler anvendes ganske ofte som kildemateriale. Herunder tager vi udgangspunkt i et sample, som er [indlæst i en buffer](../08/a-samples.md). I den sammenhæng er det oplagt at anvende UGen'en `GrainBuf` som netop læser grains fra en buffer.

Inden vi går i gang med at udforske denne UGen, skal vi indlæse et sample i en buffer. Denne buffer skal være præcis én kanal (dvs. mono). Hvis man ønsker at indlæse den ene kanal fra en stereolydfil, kan man bruge method'en `Buffer.readChannel` på følgende vis (erstat `[0]` med `[1]` for at vælge højre kanal fra lydfilen frem for venstre).
...
![Forskellen på to triggere: Impulse og Dust](../media/figures/triggere.png){ width="80%" }

Hvorfor bruge den mere uregelmæssige og kaotiske `Dust` fremfor `Impulse`? Jo, med `Impulse` kan triggeren komme til at fungere som en form for amplitude-modulation, hvor frekvensen træder frem som et hørbart artefakt med særskilt pitch. For at modvirke dette kan man anvende `Dust`, der ikke resulterer i samme periodiske mønster. Hertil kommer selvfølgelig, at man kan have æstetisk præference for det mere tilfældighedsprægede udtryk, som også kendetegner `Dust`. Er triggerfrekvensen tilstrækkeligt høj (relativt til grainvarighed) til, at der er mange samtidigt klingende grains, vil denne tilfældighed være mindre hørbar.
